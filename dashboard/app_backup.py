...
def render_auth() -> None:
    st.markdown(
        ...
        <div class="auth-wrap">
            ...
            <div class="auth-title">COVID-19 Pandemic Trend Analysis</div>
            <div class="auth-copy">
                Sign in to open the dashboard. New users can create an account.
                Passwords are stored only as bcrypt hashes in a local SQLite database.
            </div>
        </div>
        """,
    ...
    if mode == "Sign in":
        ...
        if submitted:
            ok, message = register_user(full_name, username, email, password, confirm)
            if ok:
                st.success(message)
            else:
                st.error(message)
    ...
    st.markdown(
        '<div class="footer">COVID-19 Pandemic Trend Analysis · Local demonstration application</div>',
        unsafe_allow_html=True,
    )
